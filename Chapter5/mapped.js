...
function lessPrice(inArray,action){
    let array = []
    for(let items of inArray){
        if(action(items)){
            array.push(items);
        }
    }
...
let filterArray = lessPrice(groceryItems,(items)=>{return(items.price < 60)});

// console.log(filterArray);

function mapped(inArray,action){
    let outArray = [];
    for(let array of inArray){
        if(action(array)){
            outArray.push(action(array));
        }
    }
    return outArray;
}

let mp = mapped(filterArray,(s)=>{return s.name});
